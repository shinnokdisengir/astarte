...
defmodule Astarte.Core.Generators.Triggers.Policy.ErrorType do
  ...
  def error_type(params \\ []) do
    params gen all error <-
                     one_of([error_keyword(), error_range()]),
                   params: params do
      error
    end
  end
  ...
  @doc """
  Generates a valid Astarte Triggers Policy ErrorType Format from scratch
  """
  @spec error_type_format() :: StreamData.t(ErrorType.t())
  def error_type_format, do: member_of([:map, :raw])
